...
/*   Updated: 2019/04/03 17:59:50 by lzimmerm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//		printf("HEEEEY\n");
	...
	{
//		printf("in lst_read : found \'\\n\' at pos %d \n", pos);
		*line = ft_strdup((*lst)->content); //ca malloc un peu trop...
		*line = ft_strtrimc(*line, '\n'); //ca malloc un peu trop...
		tmp = ft_strsub((*lst)->content, pos + 1, (*lst)->content_size - (pos + 1));
		free((*lst)->content);
		(*lst)->content = tmp; 
		(*lst)->content_size = (*lst)->content_size - (pos + 1);
...
//			printf("in lst_read : found \'\\n\' at pos %d \n", pos);
			...
			*line = ft_strtrimc(ft_strdup((*lst)->content), '\n'); //ca malloc un peu trop...
			free((*lst)->content);
			(*lst)->content = tmp; 
//			printf("Hellloooooooo !!!, content = %d\n", *(*lst)->content);
			(*lst)->content_size = (*lst)->content_size - (pos + 1);
//			printf("lst->content = %s, lst->content_size = %lu \n", (*lst)->content, (*lst)->content_size);
			return ;
		}
